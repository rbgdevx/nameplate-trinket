...
#
# Output: ../NameplateTrinket-v<version>.zip   (sibling of this repo, in the
#         shared addons working dir).
#
# The zip contains a single top-level folder `NameplateTrinket/`
...
ZIP_NAME="NameplateTrinket-v${version}.zip"
ZIP_PATH="$OUT_DIR/$ZIP_NAME"

echo "Packaging NameplateTrinket v${version}"
echo "   from: $SRC"
echo "   to:   $ZIP_PATH"

# --- Stage in a temp dir so the zip has a clean NameplateTrinket/ root ---------
STAGE="$(mktemp -d)"
...
if command -v zip >/dev/null 2>&1; then
  (cd "$STAGE" && COPYFILE_DISABLE=1 zip -rXq "$ZIP_PATH" "NameplateTrinket")
elif [[ -x "/c/Windows/System32/tar.exe" ]]; then
  (cd "$STAGE" && /c/Windows/System32/tar.exe -a -cf "$(cygpath -w "$ZIP_PATH")" "NameplateTrinket")
elif command -v powershell.exe >/dev/null 2>&1; then
  powershell.exe -NoProfile -Command \
    "Compress-Archive -Path '$(cygpath -w "$STAGE")\\NameplateTrinket' -DestinationPath '$(cygpath -w "$ZIP_PATH")' -Force"
else
