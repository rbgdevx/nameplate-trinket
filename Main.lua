  ...
  if isHarm(sourceFlags) ~= 0 then
    ...
    then
      ...
      if CDTimeCache[spellID] then
        ...
        if not (Data.n_MAP["Interrupt"][spellID] or Data.n_MAP["ETCCD"][spellID] or Data.n_MAP["Racial"][spellID]) then
          if
              ...
              if sel then
                if profile.Func.CC then
                  fr:SetPoint(
                    ...
                    pl,
                    "RIGHT",
                    profile.gSetting.RightxOfs + (profile.gSetting.FrameSize * 3) + profile.gSetting.FrameSize * SCNT,
                ...
                elseif profile.Func.Racial and (Data.n_MAP["Racial"][id] or Data.n_MAP["RST_Racial"][id]) then
                  fr:SetPoint(
                    "RIGHT",
                    pl,
                    "RIGHT",
                ...
                elseif profile.Func.Dispel and (Data.n_MAP["Dispel"][id] or Data.n_MAP["ETCCD"][id]) then
                  fr:SetPoint(
                    "BOTTOMRIGHT",
                    pl,
                    "TOPLEFT",
              ...
              if sel then
                if profile.Func.CC then
                  fr:SetPoint(
                    ...
                    pl,
                    "TOPRIGHT",
                    profile.gSetting.RightxOfs + profile.gSetting.FrameSize + profile.gSetting.FrameSize * floor(gn / 2),
                    profile.gSetting.yOfs - profile.gSetting.FrameSize - profile.gSetting.FrameSize * (gn % 2)
                  )
                  gn = gn + 1
                else
                  fr:SetPoint("TOP", UIParent, "TOP", 0, 100)
